...
class Defaults:
    ...
    def setter(self):
        ...
        # time_warp = int((6 / 50) / (time_step * 60))  # 60Hz
        time_warp = int((6 / 50) / (time_step * 144))  # 144Hz
        total_store = int(total_steps / time_warp) + (total_steps % time_warp > 0)  # round up int because of 0th frame
        number_of_patches = int(200)

        first_line = "g,container_radius,radius,density,container_amplitude,container_omega,mass,moment_of_inertia," +\
                     "spring_constant, coefficient_of_restitution,mu,gamma_t,damping,pos_x,pos_y,pos_z,velocity_x," +\
                     "velocity_y,velocity_z,omega_x,omega_y,omega_z,time_end,time_step,total_steps,time_warp," +\
                     "total_store,number_of_patches\n"
        self.defaults_file.writelines(first_line)
        default_values = f"{g},{container_radius},{radius},{density},{container_amplitude},{container_omega},{mass}," +\
                         ...
                         f"{omega[0]},{omega[1]},{omega[2]},{time_end},{time_step},{total_steps},{time_warp}," +\
